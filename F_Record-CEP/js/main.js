...
function startOrStopRecord() {
    ...
    if(btn.classList.contains('topcoat-button--cta')){
        ...
        if(document.getElementById("imageCount").innerHTML=="0"){
            ...
            btn.innerHTML = 'Continue Recording';
        }
...
async function outputVideo(){
    ...
    if(imageFiles.length == 0){
        alert("There is nothing, do you still want to render？");
        return;
    }
    ...
    try{
        ...
        async function output_3(p){
            ...
            if(imageFiles.length < 1000){
                alert(`Video rendered successfully ${outputVideo}`);
            }else{
                alert(`I'm honored to witness the birth of the world :) ${outputVideo}`);
            }
        }
        await output_0(0);
...
function clearProcessImages(){
    ...
    var result = confirm("Do you want to clear the recording？\nCheck if the video is rendered and you are satisfied with the result");
    if(!result){
        return;
    }
    const save_path = localStorage['savePath'];
